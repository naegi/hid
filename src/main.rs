...
const MOUSE_STEP_X: f32 = 1.0e-3;
const MOUSE_STEP_Y: f32 = 1.0e-3;
...
fn main() -> Result<(), std::io::Error> {
    ...
    if args.len() != 2 {
        ...
        println!("Usage: `{} DEVICE`, eg. `{} /dev/input/js0`", n, n);
        std::process::exit(1);
    }
    ...
    loop {
        ...
        let e = match event.event_code {
            EventCode::EV_ABS(EV_ABS::ABS_X) => {
                Some((EV_REL::REL_X, (MOUSE_STEP_X * event.value as f32) as i32))
            }
            EventCode::EV_ABS(EV_ABS::ABS_Y) => {
                Some((EV_REL::REL_Y, (MOUSE_STEP_Y * event.value as f32) as i32))
            }
            _ => None,
        };
